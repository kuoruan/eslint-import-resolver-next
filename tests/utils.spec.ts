...
describe("utils", () => {
  ...
  describe("test findAllPackages", () => {
    ...
    it("removes duplicate package paths", () => {
      mock({
        "/root6/packages/a/package.json": "{}",
        "/root6/packages/a/package.yaml": "{}",
        "/root6/packages/b/package.json": "{}",
      });

      const packages = findAllPackages("/root6", ["packages/*"]);
      expect(packages).toEqual(["/root6/packages/a", "/root6/packages/b"]);
    });
